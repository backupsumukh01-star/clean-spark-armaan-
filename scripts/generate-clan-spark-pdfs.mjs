/**
 * Generates Clan Spark branded PDFs for /public/audit and /public/whitepaper.
...
const CHAPTER_THEMES = [
  ...
  ["System description — CSP token", "BEP-20, supply, liquidity, listings"],
...
function subsectionBody(chapterIdx, subIdx, theme, subtitle) {
  ...
  return [
    `This subsection (${chapterIdx + 1}.${subIdx}) addresses ${subtitle} within the Clan Spark Extended Security & Audit Assessment dated 2026. The assessment covers Bitcoin mining operations, hosting relationships, treasury workflows, and the CSP token on BNB Smart Chain as described in the system boundary statement. Procedures performed were limited to inquiry, observation, inspection of selected artifacts, and agreed-upon analytics where noted. No opinion on financial statements is expressed.`,
...
async function makeLongAuditPdf() {
  ...
  function drawFooter() {
    if (!page) return;
    const t = `Clan Spark — Extended Security & Audit Assessment — Public Summary — Page ${pageNumber}`;
    page.drawText(t, { x: LEFT, y: 32, size: 7, font, color: MUTED });
    page.drawText("Confidential — for stakeholder review — clansparkmining.com", {
      x: LEFT,
      y: 22,
      size: 7,
      font,
  ...
  function drawCover() {
    ...
    page.drawText("Bitcoin mining operations & CSP token (BNB Smart Chain)", {
      x: LEFT,
      y: 545,
      size: 11,
      font,
...
async function makePdf({ title, subtitle, sections }) {
  ...
  drawRule();
  needSpace(2);
  page.drawText("Clan Spark — Bitcoin mining & CSP on BNB Smart Chain.", {
    x: LEFT,
    y,
    size: 9,
    font,
  ...
  y -= 12;
  page.drawText("clansparkmining.com — Information only; not investment advice.", {
    x: LEFT,
    y,
    size: 9,
    font,
...
const whitepaperSections = [
  ...
  },
  {
    ...
    body: `Clan Spark evaluates power purchase structures, curtailment programs, and where applicable lower-carbon energy sources. Public communications should avoid greenwashing; actual mix and emissions reporting will depend on host geography and contracts.`,
  },
  {
    heading: "CSP token design",
    body: `CSP uses a fixed supply with transparent allocation categories (community, liquidity, ecosystem, etc.) as shown in project tokenomics. Governance may propose uses of ecosystem allocations subject to legal and technical constraints.`,
  },
  {
    heading: "Risk factors",
    body: `Bitcoin price volatility, halving schedule, difficulty increases, regulatory change, counterparty failure, and force majeure can all affect mining economics. Token liquidity and bridge/exchange risks apply separately to CSP markets.`,
  },
  {
    heading: "Contact & updates",
    body: `Published materials will be updated as operations mature. Refer to clansparkmining.com and official social channels for notices. Verify all contract addresses via BscScan before interacting.`,
  },
];
...
const whitepaperExtendedSections = (() => {
  const topics = [
    ...
    ["Governance roadmap", "CSP voting scope and legal limitations"],
  ...
  for (let i = 0; i < topics.length; i++) {
    ...
    out.push({
      ...
      body: [
        `This section elaborates on ${sub} for Clan Spark readers who want depth beyond the homepage. It is descriptive, not prescriptive legal or investment advice.`,
        `Management may update strategies as markets, hosts, and regulations evolve. When material changes occur, Clan Spark should communicate them through official channels and, where required, file or publish notices appropriate to each jurisdiction.`,
        `CSP holders and mining counterparties should perform independent diligence. No projection of yield, payback period, or token appreciation is implied. Past network conditions do not predict future Bitcoin price or difficulty.`,
      ].join("\n\n"),
    });
  }
  return out;
...
async function main() {
  ...
  const wpPdf = await makePdf({
    ...
    subtitle: "Bitcoin mining strategy and CSP participation framework",
    sections: [
      ...whitepaperSections,
  ...
  const auditPath = path.join(auditDir, "Clan_Spark_Security_Audit_Summary.pdf");
  const wpPath = path.join(wpDir, "Clan_Spark_Whitepaper.pdf");
